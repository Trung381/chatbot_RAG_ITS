...
# def chunk_by_title(file_path: str):
#     """
#     Phân tích một file PDF hoặc DOCX và chia nhỏ nội dung dựa trên các tiêu đề (Title).
#     Hàm này cũng xử lý các mục danh sách (ListItem) và bảng (Table) một cách đặc biệt.

#     Args:
#         file_path: Đường dẫn đến file .pdf hoặc .docx.
...
#     """
#     # Để trích xuất cấu trúc bảng, chúng ta cần dùng chiến lược "hi_res"
#     # và bật cờ infer_table_structure.
#     common_kwargs = {
#         "strategy": "hi_res",
#         "infer_table_structure": True,
#     }

#     # Chọn hàm partition tùy theo loại file
#     if file_path.endswith(".pdf"):
#         # Đối với PDF, bạn có thể thêm các ngôn ngữ nếu cần, ví dụ: languages=["vie", "eng"]
#         elements = partition_pdf(file_path, **common_kwargs)
#     elif file_path.endswith(".docx"):
#         elements = partition_docx(file_path, **common_kwargs)
#     else:
#         raise ValueError("Loại file không được hỗ trợ. Vui lòng sử dụng .pdf hoặc .docx")

#     chunks = []
#     current_chunk = {"title": "Phần mở đầu", "content": ""} # Bắt đầu với một chunk mặc định

#     for el in elements:
#         if isinstance(el, Title):
#             # Nếu đã có nội dung trong chunk hiện tại, lưu nó lại
#             if current_chunk["content"].strip():
#                 chunks.append(current_chunk)
#             # Bắt đầu một chunk mới với tiêu đề mới
#             current_chunk = {"title": el.text.strip(), "content": ""}

#         elif isinstance(el, ListItem):
#             # Thêm định dạng gạch đầu dòng cho các mục trong danh sách
#             current_chunk["content"] += f"- {el.text.strip()}\n"

#         elif isinstance(el, Table):
#             # Lấy bảng dưới dạng HTML nếu có, nếu không thì lấy dạng text
...
#         elif isinstance(el, NarrativeText):
#             # Thêm văn bản tường thuật vào nội dung chunk
#             current_chunk["content"] += el.text.strip() + "\n"

#     # Đừng quên lưu chunk cuối cùng sau khi vòng lặp kết thúc
#     if current_chunk["title"] or current_chunk["content"].strip():
#         chunks.append(current_chunk)
...
# # --- VÍ DỤ SỬ DỤNG ---
# # Hãy chắc chắn rằng bạn có file tên là "test_border.pdf" trong cùng thư mục
# # hoặc thay đổi bằng đường dẫn đúng đến file của bạn.
# try:
#     file_path = "test_pageless.pdf"
...
#     print("Hãy chắc chắn rằng bạn đã cài đặt tất cả các thư viện cần thiết cho chiến lược 'hi_res'.")
...
# --- VÍ DỤ SỬ DỤNG ---
try:
    file_path = "test_pageless.html"  # thay đổi sang .pdf, .docx hoặc .html tùy bạn
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"File không tồn tại: '{file_path}'. Vui lòng tạo file này hoặc thay đổi đường dẫn.")

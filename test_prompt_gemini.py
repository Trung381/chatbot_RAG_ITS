...
# Thay đổi 1: Import thư viện của Google Gemini thay vì OpenAI
from langchain_google_genai import ChatGoogleGenerativeAI
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.output_parsers import JsonOutputParser
from langchain_core.exceptions import OutputParserException

# Tải các biến môi trường từ file .env (cần chứa GOOGLE_API_KEY)
load_dotenv()

# --- Định nghĩa System Prompt ---
# System prompt không thay đổi, vì nó mô tả logic chung cho mô hình
SYSTEM_PROMPT = f"""
    ### ROLE AND MAIN TASK
...
def get_rag_response(context: str, question: str) -> dict:
    """
    Sử dụng LangChain và Gemini để phân tích context và question, trả về OUTPUT.
    ...
    Returns:
        dict: Một dictionary Python với hai key là "content" và "code".
              Trả về một dictionary lỗi nếu có vấn đề xảy ra.
    """
    # Thay đổi 2: Kiểm tra biến môi trường của Google
    if not os.getenv("GOOGLE_API_KEY"):
        raise ValueError("GOOGLE_API_KEY chưa được thiết lập. Vui lòng tạo file .env hoặc export biến môi trường.")

    # Thay đổi 3: Khởi tạo Model Gemini
    # Sử dụng "gemini-pro" là một model mạnh mẽ và phổ biến
    # temperature=0.6 được giữ lại để câu trả lời có sự sáng tạo vừa phải nhưng vẫn bám sát ngữ cảnh
    model = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0.6, convert_system_message_to_human=True)


    # 2. Khởi tạo Output Parser để đảm bảo đầu ra luôn là JSON
    # Lưu ý: Gemini có thể cần hướng dẫn rõ hơn về JSON, nhưng thường JsonOutputParser vẫn hoạt động tốt
    ...
    # Logic của chain không thay đổi, đây là ưu điểm của LangChain
    ...
    try:
        ...
        return response
    # Gemini có thể trả về lỗi khác, nhưng Exception chung vẫn bắt được
    except Exception as e:
        print(f"Đã có lỗi không xác định xảy ra: {e}")
        # Thêm một chút thông tin về lỗi có thể từ parser
        if isinstance(e, OutputParserException):
            print("Lỗi có thể do Model không trả về đúng định dạng JSON.")
            print(f"Giá trị trả về thô từ model: {e.llm_output}")

        return {"content": "Lỗi hệ thống: Không thể xử lý yêu cầu.", "code": 0}


# --- VÍ DỤ SỬ DỤNG ---
if __name__ == "__main__":
    # Các ví dụ sử dụng không cần thay đổi
    ...
    # --- Trường hợp 5: Câu hỏi cụ thể, ngữ cảnh đầy đủ ---
    ...
    # --- Trường hợp 6: Câu hỏi cụ thể, ngữ cảnh đầy đủ ---
